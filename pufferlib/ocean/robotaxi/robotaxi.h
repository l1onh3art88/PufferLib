...
typedef struct Robotaxi Robotaxi;
...
Entity* load_map_binary(const char* filename, Robotaxi* env) {
    FILE* file = fopen(filename, "rb");
    //printf("fileanme: %s\n", filename);
    if (!file) return NULL;
    fread(&env->num_roads, sizeof(int), 1, file);
    env->num_entities = env->num_roads + env->active_agent_count;
...
int getGridIndex(Robotaxi* env, float x1, float y1) {
...
void add_entity_to_grid(Robotaxi* env, int grid_index, int entity_idx, int geometry_idx){
    if(grid_index == -1){
        return;
    }
...
void init_grid_map(Robotaxi* env){
...
void init_neighbor_offsets(Robotaxi* env) {
...
void cache_neighbor_offsets(Robotaxi* env){
    int count = 0;
...
int get_neighbor_cache_entities(Robotaxi* env, int cell_idx, int* entities, int max_entities) {
...
void spawn_new_goal(Robotaxi* env, int agent_idx){
    int legal_spawn_found = 0;
    int random_idx = rand() % env->num_roads;
    while(!legal_spawn_found){
        int rand_traj_idx = rand() % env->entities[random_idx].array_size;
        int is_lane_goal = env->entities[random_idx].type == ROAD_LANE;
        int is_valid_goal = env->entities[random_idx].traj_x[rand_traj_idx] != -10000;
        if(is_lane_goal && is_valid_goal){
            legal_spawn_found = 1;
            env->entities[agent_idx].goal_position_x = env->entities[random_idx].traj_x[rand_traj_idx];
            env->entities[agent_idx].goal_position_y = env->entities[random_idx].traj_y[rand_traj_idx];
        }
        random_idx = rand() % env->num_roads;
    }
}

void spawn_agent(Robotaxi* env, int agent_idx){
    int legal_spawn_found = 0;
    int random_idx = rand() % env->num_roads;

    while(!legal_spawn_found){
        int random_goal_idx= rand() % env->num_roads;
        int rand_traj_idx = rand() % env->entities[random_idx].array_size;
        int rand_goal_traj_idx = rand() % env->entities[random_goal_idx].array_size;
        if(random_goal_idx == random_idx) continue;

        int is_lane_goal = env->entities[random_goal_idx].type == ROAD_LANE;
        int is_lane_spawn = env->entities[random_idx].type == ROAD_LANE;
        int is_valid_goal = env->entities[random_goal_idx].traj_x[rand_goal_traj_idx] != -10000;
        int is_valid_spawn = env->entities[random_idx].traj_x[rand_traj_idx] != -10000;
        if(is_lane_goal && is_lane_spawn && is_valid_goal && is_valid_spawn){
            legal_spawn_found = 1;
            env->entities[agent_idx].x = env->entities[random_idx].traj_x[rand_traj_idx];
            ...
            env->entities[agent_idx].width = 1.3765977360055004f;
            env->entities[agent_idx].length = 2.929664749684897f;
            env->entities[agent_idx].height = 1.0f;
            env->entities[agent_idx].valid = 1;
            env->entities[agent_idx].goal_position_x = env->entities[random_goal_idx].traj_x[rand_goal_traj_idx];
            env->entities[agent_idx].goal_position_y = env->entities[random_goal_idx].traj_y[rand_goal_traj_idx];
            ...
            env->entities[agent_idx].type = VEHICLE;
        }
        random_idx = rand() % env->num_roads;
    }    
}

void init(Robotaxi* env){
    ...
    set_means(env);
    env->active_agent_indices = (int*)calloc(env->active_agent_count, sizeof(int));
    for(int i = 0; i < env->active_agent_count; i++){
        env->active_agent_indices[i] = i + env->num_roads;
        spawn_agent(env, env->active_agent_indices[i]);
    }
    // for(int i = 0; i < env->num_entities; i++){
    //     printf("type: %d\n", env->entities[i].type);
    // }
...
void free_initialized(Robotaxi* env){
    for(int i = 0; i < env->num_entities; i++){
...
void free_allocated(Robotaxi* env){
...
void move_dynamics(Robotaxi* env, int action_idx, int agent_idx){
...
void move_expert(Robotaxi* env, int* actions, int agent_idx){
    Entity* agent = &env->entities[agent_idx];
...
int checkNeighbors(Robotaxi* env, float x, float y, int* entity_list, int max_size, const int (*local_offsets)[2], int offset_size) {
...
void c_reset(Robotaxi* env){
    env->timestep = 0;
    for(int x = 0;x<env->active_agent_count; x++){
        env->logs[x] = (Log){0};
        int agent_idx = env->active_agent_indices[x];
        spawn_agent(env, agent_idx);
...
Client* make_client(Robotaxi* env){
...
void draw_agent_obs(Robotaxi* env, int agent_index){
    // Diamond dimensions
...
void draw_road_edge(Robotaxi* env, float start_x, float start_y, float end_x, float end_y){
...
void c_render(Robotaxi* env) {
    ...
    for(int i = 0; i < env->num_entities; i++) {
        ...
        if(env->entities[i].type == 1 || env->entities[i].type == 2) {
            ...
            if(agent_index == env->human_agent_idx && !env->entities[i].reached_goal) {
            ...
            if(IsKeyDown(KEY_LEFT_CONTROL) && env->human_agent_idx== agent_index){
                if(env->entities[i].reached_goal){
